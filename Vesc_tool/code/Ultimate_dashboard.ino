...
float compute_battery_remaining_lipo(int num_cells, float volts)
   ...
   float voltage[12] =   {4.2, 4.0916,  3.9485, 3.8635,  3.79, 3.736, 3.6856, 3.6508, 3.6199, 3.593, 3.558, 3.5};

   volts = volts / (float)num_cells;
   
   if (volts > 4.2) volts = 4.2;
   if (volts < 3.5) volts = 3.5;
...
// Interrupt service routing for "button 1"
// Button 1 allows the user to select Lipo vs Li-Ion
// battery type.
void IRAM_ATTR button1_isr()
  {
  button1 = 1;
...
// --------------------------------------------------------------------------------------------------
// The ESP32 reads analog voltages from 0.0V to 3.3V in 12-bits (0-4095).
// But, it's non-linear, it's noisy, and it's truncated at both ends of the scale.
// This function takes the sample 64 times to average out the noise.
// It's also recommended that you use a 0.1uF cap between the analog input and ground.
// Finally, this outputs the voltage on the specified pin based on my own calibration.
// It ends at 3.184V because anything higher than that reads 4095.
// It should be truncated at the bottom as well, but I wasn't able to calibrate that.
//
// NOTE: Since the ADC2 is shared with the WIFI module, you should not try to read ADC2
...
double ReadVoltage(byte pin)
   ...
   int i;
   double reading = 0;
   double counts[23] = {0,    861,  1013, 1269, 1400, 1596, 1773, 1982, 2155, 2270,
                        2474, 2610, 2738, 2922, 3093, 3315, 3440, 3620, 3795, 3960, 4070, 4092, 4095};
   double volts[23] = {0.0,  0.814, 0.935, 1.135, 1.24,  1.40, 1.56,  1.71,  1.84, 1.94,
                       2.11, 2.22,  2.32,  2.46,  2.588, 2.75, 2.820, 2.924, 3.013, 3.092, 3.140, 3.168, 3.184};  
   double result;
   
   for (i=0; i<64; i++) reading += analogRead(pin); // Reference voltage is 3v3 so maximum reading is 3v3 = 4095 in range 0 to 4095
   reading = reading/64.0;
  
   if(reading <= 0) reading = 0;
   else if (reading >= 4095) reading = 4095;

   for(i=0; i<22; i++)
      {
      if (reading >= counts[i] && reading <= counts[i+1]) break;
      }

   result = interp(counts[i], reading, counts[i+1], volts[i], volts[i+1]);
